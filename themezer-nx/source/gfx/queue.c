...
int HandleQueueList(Context_t *ctx){
    ...
    if (!CheckIfInstallSlotIsFree(installSlotOffset)){
        char *message = CopyTextArgsUtil("Bist du sicher, dass du die geplante Installation von %s entfernen moechtest?", targetOptions[installSlotOffset]);
        ShapeLinker_t *menu = CreateBaseMessagePopup("Element aus Warteschlange entfernen?", message);
        free(message);

        ShapeLinkAdd(&menu, ButtonCreate(POS(640, 470, 390, 50), COLOR_MAINBG, COLOR_CURSORPRESS, COLOR_WHITE, COLOR_CURSOR, 0, ButtonStyleBottomStrip, "Nein", FONT_TEXT[FSize28], exitFunc), ButtonType);
        ShapeLinkAdd(&menu, ButtonCreate(POS(250, 470, 390, 50), COLOR_MAINBG, COLOR_RED, COLOR_WHITE, COLOR_CURSOR, 0, ButtonStyleBottomStrip, "Ja", FONT_TEXT[FSize28], exitFunc), ButtonType);
...
ShapeLinker_t *CreateSideQueueMenu(){
    ShapeLinker_t *out = CreateSideBaseMenu("Geplante Installationen");
